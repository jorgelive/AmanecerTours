				...
				,success: function(respuesta) {
					if (respuesta.success){
						if(respuesta.hasOwnProperty('data')){
							if(respuesta.data.Paginastestimonio){
								var data=respuesta.data.Paginastestimonio;
								var testimonio = $('<div class="popContenedor"/>');
            					testimonio.append($('<p><?php __('nombre');?>: <strong>'+data.name+'</strong></p>'));
								testimonio.append($('<p><?php __('nacionalidad');?>: <strong>'+data.nacionalidad+'</strong></p>'));
								testimonio.append($('<p><?php __('fecha');?>: <strong>'+data.fecha+'</strong></p>'));
								testimonio.append($('<div class="contenido" id="C_'+data.id+'"><div class="texto">'+data.contenido+'</div></div>'));
								if(data.imagen){
									testimonio.find('#C_'+data.id).append($('<img src="/thumbs/index/?src='+data.imagen.path+'&w=200" />'))
									testimonio.find('#C_'+data.id).find('.texto').css({
										width:270
										,display:'block'
										,float:'left'
									})
								}
								testimonio.find('#C_'+data.id).append($('<div class="clear" />'))
								testimonio.dialog({
									title: data.name
									,modal: true
									,width: 550
				...
				<div class="indent">
                    <?php
						if(!empty($testimonios)){
							...
							?>
							<a id="boton<?php echo $testimonio['Paginastestimonio']['id'];?>" class="divVinculo ajaxLink" href="/paginastestimonios/detalle/<?php echo $testimonio['Paginastestimonio']['id'];?>/idioma:<?php echo Configure::read('Config.language');?>">
                            <div class="padding ui-widget-content ui-corner-all">
								<h3><?php echo $testimonio['Paginastestimonio']['name'];?></h3>
								<p><?php __('nacionalidad');?>: <?php echo $testimonio['Paginastestimonio']['nacionalidad'];?></p>
                                <p><?php __('fecha');?>: <?php echo $testimonio['Paginastestimonio']['fecha'];?></p>
								
                                <span class="contenido">
								<?php 
									if(isset($testimonio['Paginastestimonio']['imagen'])&&!empty($testimonio['Paginastestimonio']['imagen'])){
										$size='110';
										$crop='C';
										?>
									...
									echo $testimonio['Paginastestimonio']['contenido_resumen'];
								?>
                                </span>
							</div>
                            </a>
