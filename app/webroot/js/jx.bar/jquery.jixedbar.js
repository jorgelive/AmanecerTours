...
(function($) { // start jixedbar's anonymous function
	...
	$.fn.jixedbar = function(options) {
		...
		var defaults = { // default options
				...
				showToolbarText:"Mostrar barra",
				hideToolbarText:"Ocultar Barra"
			};
		...
		this.each(function() {
			...
			if (($.browser.msie && msieVersion==6) || ($.browser.msie && msieVersion==7)) {
               //estos son los problematicos
			...
			if (($.browser.msie && msieVersion==6) || ($.browser.msie && msieVersion==7)) { // for IE browsers
				pos = "absolute";
			...
			if ($(".jx-bar-button-right", this).exists()) { // check if there are currently an item on the right side portion of the bar
				...
				} else if ($.browser.msie && msieVersion==7) {
			...
			$("#jx-hid-con-id").html('<li alt="'+defaults.hideToolbarText+'"><a id="jx-hid-btn-id" class="' + hideIndicator + '"></a></li>');
			...
			// add bar style (theme)
			$(this).addClass("jx-bar ui-widget-content");
			...
			$("#jx-uhid-con-id").addClass("jx-show ui-widget-content");
			$("#jx-uhid-con-id").css({
				"overflow": constants["constOverflow"],
				"position": pos,
			...
			$("#jx-uhid-itm-id").html('<li alt="'+defaults.showToolbarText+'"><a id="jx-uhid-btn-id" class="' + unhideIndicator + '"></a></li>');

			// show/unhide container and button style

			if (defaults.showOnTop) { // rounded corner CSS for top positioned bar
				$("#jx-uhid-con-id").addClass("ui-corner-bottom");
			} else { // rounded corner CSS for bottom positioned bar
				$("#jx-uhid-con-id").addClass("ui-corner-top");
			}
			...
			//$("#jx-ttip-con-id").addClass("jx-bar-button-tooltip ui-widget-header ui-corner-all");
			...
			$("li", obj).hover(
				function () { // hover in method event
					$(this).removeClass("state-fuera");
					$(this).addClass("state-sobre");
					...
					if (tooltipTitle != "") { // show a tooltip if it's not empty
						...
						if ((defaults.showOnTop) && !($.browser.msie && msieVersion==6)) { // IE6 workaround; Don't add tooltip pointer if IE6
							$("<div />").addClass("jx-tool-point-dir-up").appendTo("#" + barTooltipID);
						}
							$("<div />").html(tooltipTitle).addClass("jx-bar-button-tooltip ui-widget-header ui-corner-all").appendTo("#" + barTooltipID);
			...
			$("li", $("#jx-uhid-con-id")).hover(
				...
				function () { // in/over event
					...
					if (tooltipTitle != "") { // show a tooltip if it is not empty
						...
						if ((defaults.showOnTop) && !($.browser.msie && msieVersion==6)) {
							$("<div />").addClass("jx-tool-point-dir-up").appendTo("#" + barTooltipID);
						}
			...
			$(window).resize(
				function(){
					...
					var marginLeft = centerScreen - ($(obj).width() / 2); // re-calculate and adjust bars position
			...
			$("li", obj).find("ul").each(function() {
				...
				$(this).parent().find("a:first").click(function() {
					...
					if ((button_active) && (active_button_name == $(this).attr("name"))) { // is this an active button?
						...
						$("#jx-menu-con-id").addClass("jx-nav-menu ui-widget-content ui-corner-all");
