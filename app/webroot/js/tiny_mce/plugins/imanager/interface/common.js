...
// ================================================	
	...
	// initiate iManager object
		...
		function iManager_open() {			
			...
			function vartourlstring(variable) {
				str='';
				for(prop in variable){
					str+=prop+"$"+ variable[prop]+"|";
				}
				return str;
			}
			...
			var stringvars=''
			if(wArgs){
				stringvars=vartourlstring(wArgs);
			}
			//-------------------------------------------------------------------------
			// open iManager dialog		
			new Ext.Window({
				title: 'Imagenes'
				,id:im.editor.editorId+'iManager'
				,width: 690
				,height: 520
				,layout: 'fit'
				,modal: true
				...
				,items: {
					...
					autoEl: {
						tag: 'iframe'
						,src: im.baseURL+'?editor='+im.editor.editorId+'&args='+stringvars+'&callback=iManager_callback'
						,frameBorder: 0
					}
		...
		if (rArgs) {
			if (!rArgs.action) { // no action set - image				
				...
				if (oLinkElement) { // edit exiting popup link								
					...
					} else if (im.isGecko || im.isWebKit) { // Gecko
	...
	// set image attributes
			...
			iManager.prototype.getImgParam = function (oImageElement, wArgs) {
				...
				if(oImageElement.attributes['hspace'] != null){
				...
				if(oImageElement.attributes['vspace']!= null){
			...
			iManager.prototype.getSelectedElement = function () {		
				...
				} else if (im.isGecko  || im.isWebKit) {
					var elm = null;
					var sel = im.oEditor.contentWindow.getSelection();		
