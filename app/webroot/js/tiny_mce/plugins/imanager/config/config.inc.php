<?php
	...
	$cfg['furl'] 	= false;	
															// default: true; if set to true, full url incl. domain will be added to image src
	$cfg['eliminarDeUrl'] 	= '/app/webroot'; //para cakephp
														
	$cfg['style']   = array ( 														// css styles for images ('class' => 'descr'); - please make sure that the classes exist in your css file
		'image-right' 				=> 'Imagen derecha',										// image: float left
		'image-left' 			=> 'Imagen izquierda',										// image: float right
...
/*		array (
	...
	$cfg['ilibs_dir'] 	   = array('/app/webroot/img/Pagina');						   	// image library path with slashes; absolute from root directory  e.g. '/pictures/' - please make sure that the directories have write permissions
	...
	$cfg['root_dir']    = ((@$_SERVER['DOCUMENT_ROOT'] && file_exists(@$_SERVER['DOCUMENT_ROOT'] . $_SERVER['PHP_SELF'])
	) ?	$_SERVER['DOCUMENT_ROOT'] : str_replace(dirname(@$_SERVER['PHP_SELF']), '', str_replace('\\', '/', realpath('.'))));
