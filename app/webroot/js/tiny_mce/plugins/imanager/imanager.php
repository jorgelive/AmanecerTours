...
// ============================================================
	function insertImage() {
		...
		if (!args.action) { // if not popup	mode, check whether there is a valid image selected		
			...
			<?php if (isset($cfg['eliminarDeUrl'])&&$cfg['eliminarDeUrl']==TRUE){ ?>
				args.src = args.src.replace('<?php echo $cfg['eliminarDeUrl']; ?>', "");
			<?php }	?>
		...
		<?php					
			if (!empty($_GET['callback'])) {          				
				echo "parent." . $_GET['callback'] . "('" . $_GET['editor']. "',this);\n";
			};
		?>	
		
		parent.Ext.getCmp('<?php echo $_GET['editor']; ?>iManager').close();
	...
	function init() {		
		...
		<?php
			if (isset($_GET['args'])&&!empty($_GET['args'])){
				$variables=$cad = substr ($_GET['args'], 0, -1);
				$variables=explode('|',$variables);
				$argsarray=array();
				foreach ($variables as $variable):
					$partes=explode('$',$variable);
					$argsarray[]=$partes[0].':\''.$partes[1].'\'';
				endforeach;
...
// ============================================================
	function setTitle(elm) {
		...
		parent.Ext.getCmp('<?php echo $_GET['editor']; ?>iManager').setTitle(retstr);
		return retstr;
...
// ============================================================
	function initImageArgs() {
		...
		<?php if (isset($cfg['eliminarDeUrl'])&&$cfg['eliminarDeUrl']==TRUE){ ?>
			args.src = '<?php echo $cfg['eliminarDeUrl'];?>'+args.src;
		<?php }	?>
    ...
    <div class="brdPad">
        ...
        <div class="brdPad">
          <div id="mainDiv">
            ...
            <div id="imDiv" class="showit">
              ...
              <p><strong>Andean<span class="hilight">Ways</span></strong> - este es el administrador de imagenes del sitio andeanways</p>
              <p>Una vez cargado desaparecerá este dialogo, puede volver a verlo en el boton ayuda en la esquina superior derecha</p>
              <p>No solo sirve para subir imágenes tambien para editarlas en linea así: cortarlas, ponerles marca de agua, rotarlas, girarlas, corregir el color y otras</p>
              <p><strong> <span class="hilight">Indicación: </span></strong> Las herramientas de imagen aparecerán en la parte inferior de la página. Usar la barra de desplazamiento para poderlas utilizar.</p>
              <br />
              <div>
                <div class="btnRight">
              ...
              <div class="floatWrap">
                ...
                <div class="colLeft">
                  <div style="float:left;">
                    ...
                    <div class="rowDiv">
                      <div class="btnRight btnAbsolute415">
                ...
                <div class="colRight">
                  <div style="float:left;">
                    ...
                    <div class="rowDiv">
                      <div class="btnRight btnAbsolute600">
      ...
      <div id="ftDivWrap">
        <div id="ftDiv">
          ...
          <span class="pad5">
          <input type="button" value="<?php echo $l->m('im_006'); ?>" class="btn" onClick="parent.Ext.getCmp('<?php echo $_GET['editor']; ?>iManager').close();" />
          </span>
        </div>
      </div>
      <!- // ftDivWrap -->
