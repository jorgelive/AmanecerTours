...
(function() {
	...
	*/
	Ext.ux.TinyMCE = Ext.extend( Ext.form.Field, {
		...
		minLength : 3,
		maxLengthText : 'El tamaño máximo para el campo es {0} caracteres',
		maxLength : 8,
		...
		onRender: function(ct, position) {
			...
			(function fixEditorSize() {
				...
				if (this.iHeight&&this.iWidth){
					size.width=this.iWidth;
					size.height=this.iHeight;
				}
				//end Jg
		...
		validateValue: function( value ) {
			...
			if(this.maxLength!=0){
				if(value.length > this.maxLength){
					this.markInvalid(String.format(this.maxLengthText+afterText, this.maxLength));
					return false;
				}else{
					valido=true;
				}
